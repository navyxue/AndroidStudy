...
/**
 * 在SearchActivity中实现user item之间分割线的效果。
 *
 * @author guolin
 * @since 18/8/8
 */
class SearchItemDividerDecoration(context: Context) : RecyclerView.ItemDecoration() {
    ...
    override fun onDraw(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        val loopLength = childCount - 1
        for (i in 0 until loopLength) {
            val view = parent.getChildAt(i)
            val nextView = parent.getChildAt(i + 1)
            if (nextView?.id == R.id.searchFeedItem || nextView?.id == R.id.loadingFooterLayout) { // 如果下一项是searchFeedItem或loadingFooterLayout布局，那么就可以停止绘制分割线
                return
            }
    ...
    companion object {
        ...
        private val TAG = "SearchItemDividerDecoration"
    }
